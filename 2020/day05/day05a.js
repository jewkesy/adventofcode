...
fs.readFile('./input.txt', 'utf8' , (err, puzzleInput) => {
  ...
  // Answer: 861
  console.log("Puzzle Answer:", getHighestSeatId(puzzleInput.split('\n')))
})

function getHighestSeatId(boardingPasses) {
	var highest = 0;
	for (let i = 0; i < boardingPasses.length; i++) {
		var curr = getSeatId(boardingPasses[i]);
...
var sample = ["FBFBBFFRLR", "BFFFBBFRRR", "FFFBBBFRRR", "BBFFBBFRLL"]
assert(getHighestSeatId(sample) == 820)